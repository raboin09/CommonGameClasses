...
void UProjectileActivation::Internal_AimAndShootProjectile(FVector& OutSpawnOrigin, FVector& ProjectileVelocity)
{
	...
	if (const FHitResult Impact = WeaponTrace(bShouldLineTrace, SphereTraceRadius); Impact.bBlockingHit)
	{
		...
		else if (DirectionDot < 0.5f)
		{
			...
			if (const FHitResult MuzzleImpact = WeaponTrace(bShouldLineTrace, SphereTraceRadius, MuzzleStartTrace, MuzzleEndTrace); MuzzleImpact.bBlockingHit)
			{
				bWeaponPenetration = true;
			}
		}
